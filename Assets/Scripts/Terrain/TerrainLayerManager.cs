...
namespace Terrain
{
    ...
    {
        [Header("Tilemap Layers")] [SerializeField]
        ...
        private int baseTerrainSortOrder;
        ...
        private bool autoGenerateWaterZones = true;

        [SerializeField] private bool autoGenerateColliders;

        private readonly Dictionary<Vector3Int, TerrainTileInfo> tileInfoCache = new();
        ...
        private void Start()
        {
            ...
            if (autoGenerateWaterZones) Invoke(nameof(GenerateWaterZones), 0.1f);

            if (autoGenerateColliders) Invoke(nameof(UpdateCollisionLayer), 0.2f);
        }
        ...
        {
            var cellPosition = baseTerrain.WorldToCell(worldPosition);
        ...
        {
            var cellPosition = waterLayer != null
                ? waterLayer.WorldToCell(worldPosition)
                : baseTerrain.WorldToCell(worldPosition);
        ...
        private void GenerateWaterZones()
        {
            ...
            var bounds = waterLayer.cellBounds;
            ...
            for (var y = bounds.yMin; y < bounds.yMax; y++)
            {
                ...
                var tile = waterLayer.GetTile(position);

                if (tile is WaterTile waterTile && waterTile.ShouldGenerateWaterZone()) waterTiles.Add(position);
            }
        ...
        private void FloodFillWaterGroup(Vector3Int start, List<Vector3Int> allWaterTiles, HashSet<Vector3Int> visited,
            List<Vector3Int> currentGroup)
        {
        ...
        private void CreateWaterZone(List<Vector3Int> waterTiles)
        {
            ...
            foreach (var tile in waterTiles) centerWorld += waterLayer.CellToWorld(tile);
            centerWorld /= waterTiles.Count;

            // Create water zone GameObject
            var waterZoneObj = new GameObject($"WaterZone_{waterTiles.Count}tiles");
            waterZoneObj.transform.position = centerWorld;

            // Parent it to the WaterZones container
            if (waterZonesParent != null) waterZoneObj.transform.SetParent(waterZonesParent.transform);

            // Add WaterZone component
            waterZoneObj.AddComponent<WaterZone>();

            // Set up collider to match the water area
            var polygonCollider = waterZoneObj.AddComponent<PolygonCollider2D>();
            polygonCollider.isTrigger = true;

            // Create collider points from water tiles (relative to centerWorld)
            var points = CreateColliderFromTiles(waterTiles, centerWorld);
        ...
        private List<Vector2> CreateColliderFromTiles(List<Vector3Int> tiles, Vector3 centerWorld)
        {
            ...
            var worldMin = waterLayer.CellToWorld(min);
            var worldMax = waterLayer.CellToWorld(max + Vector3Int.one);

            // Make points relative to centerWorld (local coordinates)
            return new List<Vector2>
            {
                new(worldMin.x - centerWorld.x, worldMin.y - centerWorld.y),
                new(worldMax.x - centerWorld.x, worldMin.y - centerWorld.y),
                new(worldMax.x - centerWorld.x, worldMax.y - centerWorld.y),
                new(worldMin.x - centerWorld.x, worldMax.y - centerWorld.y)
            };
        }
        ...
        private void UpdateCollisionLayer()
        {
            ...
            var bounds = baseTerrain.cellBounds;
        ...
        public void ClearWaterZones()
        {
            ...
            var existingZones = FindObjectsByType<WaterZone>(FindObjectsSortMode.None);
