...
{
    private static readonly Dictionary<VisualElement, List<IBinding>> bindings = new();
    ...
    public static void BindSlider(VisualElement element, string elementName, Func<float> valueGetter,
        Action<float> valueSetter = null)
    {
    ...
    public static void BindToggle(VisualElement element, string elementName, Func<bool> valueGetter,
        Action<bool> valueSetter = null)
    {
    ...
    public static void BindProgressBar(VisualElement element, string elementName, Func<float> valueGetter,
    ...
    public static void BindDropdown<T>(VisualElement element, string elementName, Func<List<T>> optionsGetter,
        Func<T> valueGetter, Action<T> valueSetter = null)
    {
        var dropdown = element.Q<DropdownField>(elementName);
        if (dropdown != null)
        {
            // Create a custom binding for list options that compares content
            var optionsBinding = new ListBinding<T>(optionsGetter,
                options => { dropdown.choices = options.ConvertAll(o => o.ToString()); });
            AddBinding(element, optionsBinding);

            // Update selected value (use ObjectBinding for all types to avoid generic constraints)
            var valueBinding = new ObjectBinding<T>(valueGetter,
                value => { dropdown.SetValueWithoutNotify(value.ToString()); });
            ...
            if (valueSetter != null)
            {
                void OnValueChanged(ChangeEvent<string> evt)
                {
                    ...
                    if (selectedIndex >= 0 && selectedIndex < options.Count) valueSetter(options[selectedIndex]);
                }
    ...
    {
        if (!bindings.ContainsKey(element)) bindings[element] = new List<IBinding>();
        bindings[element].Add(binding);
    }

    public static void UpdateBindings(VisualElement element)
    {
        if (bindings.TryGetValue(element, out var elementBindings))
    ...
    public static void ClearBindings(VisualElement element)
    {
        if (bindings.TryGetValue(element, out var elementBindings))
        {
            foreach (var binding in elementBindings) binding.Dispose();
