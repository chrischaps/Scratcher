...
public class TerrainGenerator : MonoBehaviour
{
    ...
    [Header("Tile Assets")] [SerializeField]
    private NormalTile[] grassTiles;
    ...
    [Header("Target Tilemaps")] [SerializeField]
    private Tilemap baseTerrain;

    [SerializeField] private Tilemap waterLayer;

    [Header("Lake Generation")] [SerializeField]
    private bool generateLakes = true;
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        var center = transform.position + new Vector3(startPosition.x, startPosition.y, 0);
        var size = new Vector3(mapWidth, mapHeight, 0);
        ...
        if (generateLakes)
        {
            ...
            {
                var lakePos = center + new Vector3(
    ...
    public void GenerateTerrain()
    {
        ...
        if (generateLakes && waterLayer != null) GenerateLakes();

        SmoothTerrain();

        // Refresh terrain manager
        terrainManager = FindObjectOfType<TerrainLayerManager>();
        if (terrainManager != null) terrainManager.RefreshAll();
    ...
    private void ApplyLevelConfiguration()
    {
        ...
        if (grid != null) levelConfig.ApplyToGrid(grid);
    ...
    {
        if (baseTerrain != null)
            baseTerrain.SetTilesBlock(new BoundsInt(-mapWidth / 2, -mapHeight / 2, 0, mapWidth, mapHeight, 1),
                new TileBase[mapWidth * mapHeight]);

        if (waterLayer != null)
            waterLayer.SetTilesBlock(new BoundsInt(-mapWidth / 2, -mapHeight / 2, 0, mapWidth, mapHeight, 1),
                new TileBase[mapWidth * mapHeight]);
        ...
        {
            terrainManager = FindObjectOfType<TerrainLayerManager>();
            if (terrainManager != null) terrainManager.ClearWaterZones();
        }
    }
    ...
    private void GenerateBaseTerrain()
    {
        ...
        {
            var position = startPosition + new Vector3Int(x - mapWidth / 2, y - mapHeight / 2, 0);

            var noiseValue = Mathf.PerlinNoise(x * noiseScale, y * noiseScale);
            var tileToPlace = GetTileFromNoise(noiseValue);

            if (tileToPlace != null) baseTerrain.SetTile(position, tileToPlace);
        }
    }
    ...
    private NormalTile GetTileFromNoise(float noiseValue)
    {
        ...
        if (Application.isEditor && Random.value < 0.01f) // Only log 1% of the time to avoid spam
            Debug.Log(
                $"Noise value: {noiseValue:F2}, Water threshold: {waterThreshold:F2}, Stone threshold: {stoneThreshold:F2}");

        if (noiseValue > stoneThreshold && stoneTiles.Length > 0) return GetRandomTile(stoneTiles);

        if (grassTiles.Length > 0) return GetRandomTile(grassTiles);
    ...
    private void GenerateLakes()
    {
        ...
        for (var i = 0; i < lakeCount; i++)
        {
            var lakeCenter = startPosition + new Vector3Int(
                Random.Range(-mapWidth / 2, mapWidth / 2),
                Random.Range(-mapHeight / 2, mapHeight / 2),
                0
            );

            var lakeSize = Random.Range(minLakeSize, maxLakeSize + 1);
    ...
    {
        var lakePositions = new List<Vector3Int>();

        // Generate circular lake
        for (var x = -size; x <= size; x++)
        for (var y = -size; y <= size; y++)
        {
            var position = center + new Vector3Int(x, y, 0);
            var distance = Vector2.Distance(Vector2.zero, new Vector2(x, y));

            if (distance <= size && Random.value > distance / size * 0.5f) lakePositions.Add(position);
        }

        Debug.Log($"Generated {lakePositions.Count} water tile positions for lake at {center}");

        // Place water tiles
        var waterTile = GetRandomWaterTile();
        if (waterTile == null)
        {
    ...
    {
        for (var pass = 0; pass < smoothingPasses; pass++) SmoothTerrainPass();
    }

    private void SmoothTerrainPass()
    {
        var tilesToUpdate = new Dictionary<Vector3Int, TileBase>();

        for (var x = -mapWidth / 2; x < mapWidth / 2; x++)
        for (var y = -mapHeight / 2; y < mapHeight / 2; y++)
        {
            var position = new Vector3Int(x, y, 0);
            var currentTile = baseTerrain.GetTile(position);

            // Count neighboring tile types
            var neighborCounts = CountNeighbors(position);
            ...
            {
                var newTile = GetTileOfType(mostCommonType);
                if (newTile != null) tilesToUpdate[position] = newTile;
            }
        }

        // Apply updates
        foreach (var kvp in tilesToUpdate) baseTerrain.SetTile(kvp.Key, kvp.Value);
    }

    private Dictionary<Type, int> CountNeighbors(Vector3Int position)
    {
        ...
        {
            var neighborTile = baseTerrain.GetTile(neighborPos);
            if (neighborTile != null)
            {
                var tileType = neighborTile.GetType();
    ...
    private TileBase GetTileOfType(Type tileType)
    {
        if (tileType == typeof(NormalTile)) return GetRandomTile(grassTiles);

        if (grassTiles.Length > 0 && grassTiles[0].GetType() == tileType) return GetRandomTile(grassTiles);

        if (stoneTiles.Length > 0 && stoneTiles[0].GetType() == tileType) return GetRandomTile(stoneTiles);

        if (sandTiles.Length > 0 && sandTiles[0].GetType() == tileType) return GetRandomTile(sandTiles);
    ...
    {
        var minX = startPosition.x - mapWidth / 2;
        var maxX = startPosition.x + mapWidth / 2;
        var minY = startPosition.y - mapHeight / 2;
        var maxY = startPosition.y + mapHeight / 2;
